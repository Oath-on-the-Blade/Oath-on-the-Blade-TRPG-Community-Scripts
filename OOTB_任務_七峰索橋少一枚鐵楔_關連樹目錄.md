...
- 南橋鏈各節點只讀其直接前作相應 `southbridge_*` state；無前作紀錄均可完整運行，前作 state 只形成正文已列的合作、准入、見證或時點 overlay，不改寫新事件真相。
...
- 《南橋封冊架三枚交班木楔…》只讀 `southbridge_seal_cord_*`；可建立 `southbridge_shelf_wedge_chain_restored`、`southbridge_shelf_wedge_marking_rule`、`southbridge_shelf_wedge_safe_hold`、`southbridge_shelf_wedge_incident`、`southbridge_shelf_wedge_unresolved`、`southbridge_shelf_wedge_seized`。
...
- 《南橋放車門四枚封冊簽有一枚穿在舊繩上》任何 ending 或無紀錄都可進《南橋封冊架三枚交班木楔有一枚刻了兩道線》；前作 state 只依後作正文形成 overlay。
