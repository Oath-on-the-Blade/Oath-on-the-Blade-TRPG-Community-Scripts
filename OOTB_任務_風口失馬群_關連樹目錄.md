...
- `beicao_transfer_yard_short_closure = true` 是短期 state；只有當前存檔仍顯示普通檢修尚未完成時，後作才可把它當作有效 overlay，不得永久化。
...
- 本篇目前沒有預寫下一個關連節點；任何 ending 都不因出自本篇而自動關閉既有樹內節點。日後若新篇直接讀取上述 state，應把《北槽轉運場十五枚車轄有四枚先裂了》列為直接來源。

## 維護

本樹穩定根是《風口失馬群》；不可因後續《北槽借印》或其他節點較突出而另立第二份樹目錄文檔。新增後續若直接讀取《北槽轉運場第十三秤總是輕三斤》的專用 state，應把它列作直接來源；若只共享白草驛／北槽地區，則不構成直接關連。

新增後續若直接讀取《北槽轉運場十五枚車轄有四枚先裂了》的車轄召回、責任或短期輕車運力 state，應把該篇列作直接來源；不得越過它只掛更早祖先作方便 anchor。
